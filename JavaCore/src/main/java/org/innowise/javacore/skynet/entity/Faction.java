...
public class Faction implements Runnable {
  ...
  public Faction(String name, int cycles, LinkedList<Part> externalPartsStorage,
      DayState cycleState,
  ...
  private int constructBots() {
    ...
    int botsToBuild = (int) Math.floor(
        (double) minParts / PartType.values()[minPos].getPartsRequired());
    if (botsToBuild < 1) {
      return 0;
    }
    for (int i = 0; i < partsAmount.length; i++) {
